...
function baseStore(){return {version:4,defaults:{...DEFAULTS},days:{},lastTab:'today',statsRange:7}}
...
function load(){
  ...
  let out;
  if(loaded&&loaded.days){out={...baseStore(),...loaded,version:4}}
  else{
    out=baseStore();
    try{
...
function timeline(day,c){
  return defs.map(d=>{
    const st=statusFor(day,d),label=st==='actual'?'ACTUAL':st==='partial'?'PARTIAL':st==='updated'?'UPDATED':'PLANNED';
...
function nextEvent(day,c){
  ...
  const fresh=pending.filter(e=>e.time>=now-45).sort((a,b)=>a.time-b.time);
  if(fresh.length)return fresh[0];
  const dream=pending.find(e=>e.key==='dream');
  if(dream&&now>=BED_MAX)return dream;
...
function hero(day,c){
  ...
  if(n.tomorrow){
    ...
    $('#nextMeta').textContent=`Today is wrapped. Tomorrow starts around ${fmt(n.time)}, with the first feed around ${fmt(n.feedTime)}.`;btn.classList.add('hidden');return;
  }
  ...
  $('#nextMeta').textContent=n.runningNap&&delta<0?`Nap ${n.runningNap} is running long by ${dur(Math.abs(delta))}. A paci/resettle does not end the nap — use “Woke up” when she is actually up.`:delta>5?`In about ${dur(delta)}. Tap below when it actually happens.`:delta>=-20?'Right around now. Log the real time and everything after it adjusts.':`Planned ${dur(Math.abs(delta))} ago. Log it now if it just happened, or tap the schedule row to correct it.`;
  btn.textContent=`Log ${n.label.toLowerCase()} now`;btn.dataset.key=n.key;btn.dataset.label=n.label;
}
function check(c){
  let msg='Everything currently fits the routine.',cls='';
...
function renderToday(){
  ...
  $('#sleepNowLabel').textContent=running?'Nap in progress':nextNap?`Nap ${nextNap} started`:'No nap left';$('#sleepNowSub').textContent=running?'Use Woke up when the nap ends':nextNap?'Start the next nap':'All 3 naps started';$('#sleepNowBtn').disabled=closed||!!running||!nextNap;
...
function openHistory(k){
  const d=data.days[k];$('#historyTitle').textContent=dateLabel(k);const naps=actualNapTotal(d),rows=[];
...
function renderStats(){
  const range=data.statsRange||7,days=statsDays(range),wake=[],first=[],bed=[],nap=[],gaps=[],resettles=[];
  days.forEach(([,d])=>{if(has(d,'wake'))wake.push(actualMin(d,'wake'));if(has(d,'feed1'))first.push(actualMin(d,'feed1'));if(has(d,'bed'))bed.push(actualMin(d,'bed'));const n=actualNapTotal(d);if(n)nap.push(n.minutes);gaps.push(...actualFeedGaps(d));resettles.push(resettleCount(d))});
  const totalResettles=resettles.reduce((a,b)=>a+b,0);
